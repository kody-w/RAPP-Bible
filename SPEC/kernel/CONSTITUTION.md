<!-- MIRRORED FROM https://github.com/kody-w/RAPP/blob/main/CONSTITUTION.md — DO NOT EDIT HERE; edit upstream and re-sync. -->
...
Door type is deterministic from kind: `twin`, `operator`, `personal`, `project`, `memorial`, `pre-founder`, `mirror`, `experiment`, `place`, and `custom` → `front_door` (a single AI presence). Everything else (`neighborhood`, `ant-farm`, `braintrust`, `workspace`, `hatched`, `rapplication`, `prototype`) → `gate` (a community AI you enter to find others). **Amendment (2026-06-02):** the single-presence set was extended beyond the original 2026-05-09 freeze (`twin`/`operator`) to ratify the kinds already emitted by `kind`-aware consumers — `TWIN_LIFECYCLE_SPEC.md` and `NEIGHBORHOOD_EGG_SPEC.md` (`project`), `ECOSYSTEM.md` §3/§15 (`personal`, `place`, `mirror`, `experiment`, `custom`), RAR `@rapp/twin_agent` (`personal`, `pre-founder`, `memorial`, `project`, `place`, `custom`), and RAPP-Network `project_twin_agent.py` (`project`) — which the frozen list previously contradicted. Adding a new kind requires amending this article — every consumer's behavior derives from this token, so the set cannot drift silently; the canonical machine-readable set is `VALID_KINDS` / `_FRONT_DOOR_KINDS` in `tools/door_address.py`.
