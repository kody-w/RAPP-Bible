...
> *Renders the `the_one_agent`, `required_actions`, `operator_journeys`, and `specialist_agents` sections of [`ecosystem-spec.json`](https://raw.githubusercontent.com/kody-w/rapp-god/main/api/v1/ecosystem-spec.json) v1.1.0.*
...
*Authority: `ecosystem-spec.json` v1.1.0 (`the_one_agent` + `required_actions` + `operator_journeys` + `specialist_agents`). The agent's live action enum is the executable contract; this page renders it for a human reader.*
